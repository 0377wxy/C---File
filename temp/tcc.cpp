...
{
    int temp_num, temp_time;
    cout << "请输入车牌号  到达时间" << endl;
    cin >> temp_num >> temp_time;               // 读入车辆信息
    Car *temp_c = new Car(temp_num, temp_time); // 创建车的对象
...
void Vehicle_leaving(vector<Car *> &garage, Waiting_queue *wait, int nu, int time) // 车辆离开
{
    ...
    cout << "车牌号  停车时长" << endl;
    cout << temp->num << " " << time - temp->Storage_time << endl;
    garage.pop_back();
...
void show_Waiting_situation(vector<Car *> &garage, Waiting_queue *wait) // 显示出停车场内及便道上的停车情况
{
    ...
    while (temp != NULL)
    {
        cout << temp->num << " " << temp->Time_of_arrival << endl;
        temp = temp->next;
    }
    cout << "车库内的情况：" << endl;
    ...
    {
        cout << garage[i]->num << " " << garage[i]->Time_of_arrival << " " << garage[i]->Storage_time << endl;
    }
}
...
int main()
{
    ...
    while (1)
    {
        ...
        {
            cout << "请输入出库的车牌号及当前时间" << endl;
            int a, b;
            cin >> a >> b;
            Vehicle_leaving(garage, wait, a, b);
        }
        else if (f == 3)
        {
