...
public:
    Node<T>(T min, T max, T nil) : main_data(min), high(max), nil_data(nil) { Max = high; }
    T get_data()
    {
        return main_data;
    }
    ...
    void set_max(Node<T> *a, Node<T> *b)
    {
        ;
    }
    Node<T> *get_left()
...
public:
    ...
    {
        NIL = new Node<T>(nil_data, nil_data, nil_data);
        NIL->set_color("B");
        NIL->set_max(nil_data);
    }
    ~Tree<T>()
    {
    ...
    void insert(T data, T high)
    {
        Node<T> *new_pode = new Node<T>(data, high, NIL->get_data());
        ...
        while (temp != NIL)
        {
            if (temp->get_max() < new_pode->get_max())
            {
                temp->set_max(max(new_pode->get_max(), temp->get_max()));
            }
    ...
    void RB_delete(T data)
    {
        ...
        else
        {
            ...
            {
                transplant(y, x);
                x->get_father()->set_max(max(max(x->get_father()->get_right()->get_max(), x->get_max()), x->get_high()));
            ...
            y->set_max(max(max(y->get_left()->get_max(), y->get_right()->get_max()), y->get_high()));
            max_rdjust(x->get_father());
        }
    ...
    void left_rotate(Node<T> *x)
    {
        ...
        x->set_max(max(max(x->get_left()->get_max(), y->get_left()->get_max()), x->get_high()));
        y->set_max(max(max(x->get_max(), y->get_right()->get_max()), y->get_high()));
    ...
    {
        Node<T> *y = x->get_left();
        x->set_max(max(max(x->get_right()->get_max(), y->get_right()->get_max()), x->get_high()));
        y->set_max(max(max(x->get_max(), y->get_left()->get_max()), y->get_high()));
    ...
    void max_rdjust(Node<T> *x)
    {
        if (x == NIL)
        {
            return;
        }
        x->set_max(max(max(x->get_left()->get_max(), x->get_right()->get_max()), x->get_high()));
        max_rdjust(x->get_father());
    }
    Node<T> *interval_search(T start, T end)
    {
        Node<T> *x = root;
        while (x != NIL && (x->get_data() > end || x->get_high() < start))
        {
            if (x->get_left() != NIL && x->get_left()->get_max() > start)
            {
                x = x->get_left();
            }
            else
...
int main()
{
    ...
    //tree->RB_delete(13);
    //tree->RB_delete(7);
    cout << tree->interval_search(12, 13)->get_data() << endl;
    cout << "   xxx x  " << endl;
    delete tree;
